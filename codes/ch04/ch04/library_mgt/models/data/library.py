...
class Book:
    ...
    def __init__(self, book_id: int, title: str, classification: Classification, author: str, year_published: datetime,
                 edition: int):
    ...
    def __repr__(self):
        return ' '.join(
            [str(self.book_id), self.classification, self.author, self.year_published.strftime("%m/%d/%Y, %H:%M:%S"),
             self.edition])
...
class BookRequest:
    ...
    def __repr__(self):
        return ' '.join(
            [str(self.req_id), str(self.book_id), self.request_date.strftime("%m/%d/%Y, %H:%M:%S"), str(self.status)])
...
class BookIssuance:
    ...
    def __repr__(self):
        return ' '.join(
            [str(self.issue_id), str(self.req_id), self.approved_by, self.approved_date.strftime("%m/%d/%Y, %H:%M:%S")])
