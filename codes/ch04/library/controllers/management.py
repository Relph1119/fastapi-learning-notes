...
def request_book(request: BookRequestReq):
    book_request = BookRequest(status=request.status, request_date=request.book_id, book_id=request.book_id,
                               req_id=uuid4().int)
...
def approve_request(approval: BookIssuanceReq):
    book_approval = BookIssuance(approved_by=approval.approved_by, approved_date=approval.approved_date,
                                 req_id=approval.req_id, issue_id=uuid4().int)
