...
def add_profile(req: ProfileReq, credentials: HTTPBasicCredentials = Depends(http_basic),
...
def update_profile(id: int, req: ProfileReq, credentials: HTTPBasicCredentials = Depends(http_basic),
                   sess: Session = Depends(sess_db)):
    profile_dict = req.dict(exclude_unset=True)
    repo: ProfileRepository = ProfileRepository(sess)
