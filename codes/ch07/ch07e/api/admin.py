...
def list_signup(current_user: Login = Security(get_current_valid_user, scopes=["admin_read"]),
...
def update_signup(id: int, req: SignupReq,
...
def delete_signup(id: int, current_user: Login = Security(get_current_valid_user, scopes=["admin_write"]),
                  sess: Session = Depends(sess_db)):
    repo: SignupRepository = SignupRepository(sess)
...
def get_signup(id: int, current_user: Login = Security(get_current_valid_user, scopes=["admin_read"]),
               sess: Session = Depends(sess_db)):
    repo: SignupRepository = SignupRepository(sess)
