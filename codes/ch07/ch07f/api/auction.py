...
def add_auction(req: AuctionsReq, current_user: Login = Security(get_current_valid_user, scopes=["auction_write"]),
...
def update_auction(id: int, req: AuctionsReq,
...
def delete_auction(id: int, current_user: Login = Security(get_current_valid_user, scopes=["auction_write"]),
                   sess: Session = Depends(sess_db)):
    repo: AuctionsRepository = AuctionsRepository(sess)
...
def list_all_auction(current_user: Login = Security(get_current_valid_user, scopes=["auction_read"]),
                     sess: Session = Depends(sess_db)):
    repo: AuctionsRepository = AuctionsRepository(sess)
...
def get_auction(id: int, current_user: Login = Security(get_current_valid_user, scopes=["auction_read"]),
                sess: Session = Depends(sess_db)):
    repo: AuctionsRepository = AuctionsRepository(sess)
