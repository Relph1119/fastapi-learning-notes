...
@router.post("/bid/add")
def add_bid(req: BidsReq, current_user: Login = Security(get_current_valid_user, scopes=["bidder_write"]),
...
def update_bid(id: int, req: BidsReq, current_user: Login = Security(get_current_valid_user, scopes=["bidder_write"]),
               sess: Session = Depends(sess_db)):
    bid_dict = req.dict(exclude_unset=True)
    repo: BidsRepository = BidsRepository(sess)
...
def delete_bid(id: int, current_user: Login = Security(get_current_valid_user, scopes=["bidder_write"]),
               sess: Session = Depends(sess_db)):
    repo: BidsRepository = BidsRepository(sess)
...
@router.get("/bid/all")
def list_all_bid(current_user: Login = Security(get_current_valid_user, scopes=["bidder_read"]),
                 sess: Session = Depends(sess_db)):
    repo: BidsRepository = BidsRepository(sess)
...
@router.post("/bid/{id}")
def get_bid(id: int, current_user: Login = Security(get_current_valid_user, scopes=["bidder_read"]),
            sess: Session = Depends(sess_db)):
    repo: BidsRepository = BidsRepository(sess)
