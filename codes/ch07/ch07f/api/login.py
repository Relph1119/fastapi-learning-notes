...
def signup_approve(username: str, current_user: Login = Security(get_current_valid_user, scopes=["admin_write"]),
...
def access_token(code: str = Form(...), grant_type: str = Form(...), redirect_uri: str = Form(...),
                 sess: Session = Depends(sess_db)):
...
def authorizationUrl(state: str, client_id: str, redirect_uri: str, scope: str, response_type: str,
                     sess: Session = Depends(sess_db)):
    global state_server
    state_server = state
    ...
    if authenticate(account.username, account.password, account):
        return RedirectResponse(
            url=redirect_uri + "?code=" + auth_code + "&grant_type=" + response_type + "&redirect_uri=" + redirect_uri + "&state=" + state)
    else:
...
def list_all_login(current_user: Login = Security(get_current_valid_user, scopes=["admin_read"]),
                   sess: Session = Depends(sess_db)):
    loginrepo = LoginRepository(sess)
