...
def add_profile(req: ProfileReq,
                current_user: Login = Security(get_current_valid_user, scopes=["bidder_write", "buyer_write"]),
...
def list_all_profile(current_user: Login = Security(get_current_valid_user, scopes=["bidder_read", "buyer_read"]),
                     sess: Session = Depends(sess_db)):
    repo: ProfileRepository = ProfileRepository(sess)
...
def get_profile(id: int, current_user: Login = Security(get_current_valid_user, scopes=["bidder_read", "buyer_read"]),
                sess: Session = Depends(sess_db)):
    repo: ProfileRepository = ProfileRepository(sess)
